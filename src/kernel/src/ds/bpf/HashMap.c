...
static void hashmap_add_entry(struct hashmap_entry ** pprev, struct hashmap_entry * entry) {
    entry->next = *pprev;
    *pprev = entry;
}

static void hashmap_del_entry(struct hashmap_entry ** pprev, struct hashmap_entry * entry) {
    *pprev = entry->next;
    entry->next = NULL;
}
...
static int hashmap_grow(struct HashMap * map) {
    ...
    if (new_cap_bits < HASHMAP_MIN_CAP_BITS) new_cap_bits = HASHMAP_MIN_CAP_BITS;
...
int hashmap__insert(struct HashMap * map,
    ...
    if (strategy != HASHMAP_APPEND && hashmap_find_entry(map, key, h, NULL, &entry)) {
        if (old_key) *old_key = entry->key;
    ...
    if (strategy == HASHMAP_UPDATE) return -1;
...
bool hashmap__find(const struct HashMap * map, const void * key, void ** value) {
    ...
    h = hash_bits(map->hash_fn(key, map->ctx), map->cap_bits);
    if (!hashmap_find_entry(map, key, h, NULL, &entry)) return false;
...
bool hashmap__delete(struct HashMap * map,
    ...
    h = hash_bits(map->hash_fn(key, map->ctx), map->cap_bits);
    if (!hashmap_find_entry(map, key, h, &pprev, &entry)) return false;
