...
struct VPArrayList * vpa_create(uint32_t initial_cap) {
    ...
    *list = (VPArrayList){
        .length = 0, .capacity = initial_cap, .array = kmalloc(initial_cap * sizeof(void *))};
...
void vpa_free(VPArrayList * arr, FreeFunc freefunc) {
    if (freefunc != NULL) {
        for (uint32_t i = 0; i < arr->length; i++) { freefunc(arr->array[i]); }
    }
...
void vpa_resize(VPArrayList * list, uint32_t new_size, FreeFunc freeFunc) {
    ...
    if (list->length > list->capacity) {
        if (freeFunc != NULL) {
            for (uint32_t i = list->length; i > list->capacity; i--) { freeFunc(list->array[i]); }
        }
