...
//need some way to read a label from linker script
static unsigned int* first_level_pt = (unsigned int* const)0x10000;
...
void mmap(void){
	...
	//map first_level_pt where its at (ie page 0)
	first_level_pt[0] = 0<<20 | 0x0800 | 2;

	//map the kernel where its currently loaded in the same location temporarily
	//should be less than a MB
	first_level_pt[0x100000>>20] = 0x100000 | 0x0400 | 2;

	//also map it to high memory at 0xf0000000 (vpn = 3840)
	first_level_pt[0xf0000000>>20] = 0x100000 | 0x0400 | 2;
	...
	first_level_pt[0x80100000>>20] = 0x10000000 | 0x0400 | 2;
	...
	register unsigned int pt_addr = (unsigned int)first_level_pt;
	...
	//add kernel offest to pc and store in r3	
	asm volatile("ldr r3, [pc, #0xf0000000]");
	//jump to the same code running at the address 
	//where the kernel is also loaded in high vm 
	asm volatile("bx r3");	

	//if the code reaches here then we're now in virtual address space
	//remove kernel image from low address
	first_level_pt[0x100000>>20] = 0;
