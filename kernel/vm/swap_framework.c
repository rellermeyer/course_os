...
uint32_t store_page_LZ(void *page, uint32_t *ID){
	...
	if (curr_ent->cmp_page != NULL) {
		curr_ent->next = (struct swap_entry*) kmalloc(sizeof(struct swap_entry));
		i++;	
	}
	*ID = i; 
//	curr_ent->cmp_size = cmp_size; IF NEEDED 
	...
	// os_memcpy(*dest whatever it may be, *cmp_page, cmp_size)
	// curr_ent->e_page = *dest  after deciding dest, store location of compressed page in entry; i
	// may need to remember old page ID for some reason?
	
//	kfree(cmo_page); // after copying over relevant bits of cmp data in this oversized buffer
	memory_count += 4096;
	return *ID;
}
